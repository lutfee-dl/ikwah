...
export default function LoanAddModal({ onClose }: LoanAddModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300" onClick={() => onClose()}>
      <div 
        className="bg-white w-full sm:max-w-xl sm:rounded-[2.5rem] rounded-t-[2.5rem] shadow-2xl overflow-hidden animate-in slide-in-from-bottom sm:zoom-in-95 duration-300 max-h-[95svh] sm:max-h-[90vh] flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="bg-gradient-to-r from-amber-500 to-orange-600 p-6 sm:p-8 text-white relative shrink-0">
          ...
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 sm:w-14 sm:h-14 bg-white/20 rounded-2xl flex items-center justify-center backdrop-blur-md">
              <Landmark size={28} />
            </div>
            <div>
              <h2 className="text-xl sm:text-2xl font-black tracking-tight">ออกสินเชื่อใหม่</h2>
              <p className="text-amber-100 text-xs sm:text-sm font-medium opacity-80">สร้างสัญญาและเบิกจ่ายเงินกู้โดยตรง</p>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="p-6 sm:p-8 bg-white border-t border-slate-100 flex gap-3 shrink-0">
            <button
              type="button"
              onClick={() => onClose()}
              className="flex-1 px-4 py-4 bg-slate-100 text-slate-600 rounded-2xl text-sm font-black hover:bg-slate-200 transition-all"
            >
              ยกเลิก
            </button>
            <button
              ...
              className="flex-[2] px-4 py-4 bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-600 hover:to-orange-700 text-white rounded-2xl text-sm font-black shadow-lg shadow-amber-200 disabled:opacity-50 flex items-center justify-center gap-2 transition-all"
            >
